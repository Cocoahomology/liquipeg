...
export const formatBridgesData = ({
  ...
  if (chain) {
    filteredBridges = filteredBridges.filter(({ chains = [] }) =>
  ...
  filteredBridges = filteredBridges.map((bridge) => {
    ...
    bridge.change_1d = getPercentChange(
      bridge.lastDailyVolume,
      bridge.dayBeforeLastVolume
    );
  ...
  filteredBridges = filteredBridges.sort(
    (a, b) => b.lastDailyVolume - a.lastDailyVolume
  );
