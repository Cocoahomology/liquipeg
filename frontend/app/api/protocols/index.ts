...
export async function fetchRawProtocolsData() {
  ...
  const protocolResults = await Promise.allSettled(
    protocols
      .filter((protocol) => !PROTOCOLS_TO_IGNORE.includes(protocol.protocolId))
      .map(async (protocolData) => {
        ...
        const chainResults = await Promise.allSettled(
          chains.map(async (chainName) => {
            ...
            const tmResults = await Promise.allSettled(
              chainImmutables.troveManagers.map(
                async ({ troveManagerIndex }) => {
                  ...
                  protocolDataByName[protocolId].chainData[
                    chainName
                  ].troveManagers[troveManagerIndex] = {
                    troveManagerIndex,
                    poolDataPoints,
                    priceDataPoints,
                    colImmutables:
