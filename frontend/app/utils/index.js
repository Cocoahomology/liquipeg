...
export const toNiceDayMonthYear = (date) =>
  dayjs.utc(dayjs.unix(date)).format("DD MMM YYYY");

export const timeFromNow = (date) => dayjs.utc(dayjs.unix(date)).fromNow();

export function formatUnlocksEvent({
  description,
  noOfTokens,
  timestamp,
  price,
  ...
  description = description?.replace(
    "{timestamp}",
    `${toNiceDateYear(timestamp)} (${timeFromNow(timestamp)})`
  );
  return description;
}
...
export const toK = (num) => {
  ...
  if ([4, 5, 6].includes(stringifiedNum.length)) {
    return (
      (+stringifiedNum / 1_000).toLocaleString(undefined, {
  ...
  if ([7, 8, 9].includes(stringifiedNum.length)) {
    return (
      (+stringifiedNum / 1_000_000).toLocaleString(undefined, {
        maximumFractionDigits: 2,
      }) + "m"
    );
  }
  ...
  if ([10, 11, 12].includes(stringifiedNum.length)) {
    return (
      (+stringifiedNum / 1_000_000_000).toLocaleString(undefined, {
  ...
  if (stringifiedNum.length > 12) {
    return (
      (+stringifiedNum / 1_000_000_000_000).toLocaleString(undefined, {
        maximumFractionDigits: 3,
      }) + "t"
    );
  }
  ...
  return num.toLocaleString(undefined, {
    maximumFractionDigits:
      num > 0.1 ? 1 : num > 0.01 ? 2 : num > 0.0001 ? 3 : 5,
  });
};
...
export const formattedNum = (number, symbol = false) => {
  ...
  if (symbol) {
    return `${currencyMark}${num.toLocaleString(undefined, {
      maximumFractionDigits:
        num > 0.1 ? 2 : num > 0.01 ? 3 : num > 0.0001 ? 4 : 5,
    })}`;
  }
...
export const filterCollectionsByCurrency = (collections, displayUsd) =>
  (collections &&
    ...
    collections.map((collection) => ({
      ...
      dailyVolume: displayUsd
        ? collection?.dailyVolumeUSD
        : collection?.dailyVolume,
      totalVolume: displayUsd
        ? collection?.totalVolumeUSD
...
export function formattedPercent(
  ...
  return (
    <span
      ...
      style={{ "--weight": fontWeight, "--color": color }}
    >
      {finalValue}
    </span>
  );
...
export const getTokenDominance = (topToken, totalVolume) => {
  const dominance =
    topToken.tvl && totalVolume && (topToken.tvl / totalVolume) * 100.0;
